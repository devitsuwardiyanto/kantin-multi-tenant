...
class CheckoutServiceTest extends TestCase
{
    ...
    public function test_checkout_at_commission_switch_boundary_never_lacks_a_scheme(): void
    {
        $canteen = Canteen::factory()->create();
        $tenant = $this->tenantWithCommission($canteen, 0.15);
        $switchAt = now()->addDay()->startOfSecond();
        app(ChangeCommissionSchedule::class)->handle($tenant, 0.20, $switchAt);
        $menu = Menu::factory()->create(['tenant_id' => $tenant->id, 'base_price' => 10000, 'stock_qty' => 10]);

        // Satu detik sebelum pergantian → skema lama; tepat saat pergantian → skema baru.
        foreach ([[-1, 1500], [0, 2000]] as [$offsetSeconds, $expectedCommission]) {
            $this->travelTo($switchAt->addSeconds($offsetSeconds));
            $session = $this->sessionFor($canteen);
            $this->cart()->add($session, $menu->id, 1);

            $tenantOrder = $this->checkout()->checkout($session, (string) Str::uuid())->order->tenantOrders()->firstOrFail();

            $this->assertSame($expectedCommission, $tenantOrder->commission_amount);
        }
    }
