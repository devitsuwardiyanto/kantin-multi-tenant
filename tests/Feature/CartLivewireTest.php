...
class CartLivewireTest extends TestCase
{
    ...
    public function test_customer_landing_renders_catalog_and_cart_module_components(): void
    {
        $canteen = Canteen::factory()->create();

        $this->get(route('customer.home', ['canteen' => $canteen->slug]))
            ->assertOk()
            ->assertSeeLivewire('catalog::menu-catalog')
            ->assertSeeLivewire('ordering::cart');
    }
