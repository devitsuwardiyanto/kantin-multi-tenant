...
class KitchenBoardLivewireTest extends TestCase
{
    ...
    public function test_non_member_cannot_open_board(): void
    {
        ...
        Livewire::actingAs($outsider)
            ->test('kitchen::kitchen-board', ['tenantId' => $tenantB->id])
            ->assertForbidden();
    }
    ...
    public function test_member_sees_paid_order_and_advances_status(): void
    {
        ...
        Livewire::actingAs($member)
            ->test('kitchen::kitchen-board', ['tenantId' => $tenant->id])
    ...
    public function test_kitchen_page_renders_kitchen_module_component(): void
    {
        [$tenant, $member] = $this->paidOrderWithMember();

        $this->actingAs($member)
            ->get(route('tenant.kitchen', ['tenant' => $tenant->slug]))
            ->assertOk()
            ->assertSee('Kitchen Display')
            ->assertSeeLivewire('kitchen::kitchen-board');
    }
}
