...
```
- Route milik modul (`app/Modules/*/routes/*.php`) ikut ter-cache oleh `route:cache` (diverifikasi, termasuk route
  closure); channel broadcast `tenant.{tenantId}.orders` didaftarkan `KitchenServiceProvider` sehingga tetap aktif saat
  route di-cache. Setelah mengubah file route modul di server, jalankan ulang `php artisan route:cache`.
...
- Webhook pembayaran: verifikasi **signature HMAC atas raw body** + dedup + **rate limit** (`throttle:qris-webhook`); CSRF dikecualikan hanya untuk `webhooks/*`. Pengecualian CSRF dan limiter didaftarkan `PaymentsServiceProvider` (modul pemilik endpoint).
- Rate limit scan QR (`throttle:qr-scan`, didaftarkan `OrderingServiceProvider`); cookie `order_tracking` dikecualikan dari enkripsi oleh modul Ordering.
